        ...
        reflectionLine@ for (reflectionX in 0 until maxX) {
            ...
            for (x in 0..maxX) {
                for (y in 0..maxY) {
                    ...
                    if (map[reflectedPoint] != null && map[Point(x, y)] != map[reflectedPoint]) {
                        if (smudge == null) smudge = Point(x, y)
                        else if (smudge == reflectedPoint) continue
                        else continue@reflectionLine
                    }
                }
            }
            if (smudge == null) continue@reflectionLine
